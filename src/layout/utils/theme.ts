...
export const DEFAULT_THEME_PRESET = 'Lara';
export const DEFAULT_THEME_PRIMARY = 'green';
...
export const surfaces: ThemePaletteOption[] = [
    {
        name: 'slate',
...
export function resolveThemePresetName(name?: string | null): ThemePresetName {
    return name && name in themePresets ? (name as ThemePresetName) : DEFAULT_THEME_PRESET;
}

export function resolveThemePreset(name?: string | null) {
    return themePresets[resolveThemePresetName(name)];
}

export function findPrimaryColor(name?: string | null) {
    return primaryColors.find((color) => color.name === name);
}

export function resolveThemePrimaryName(name?: string | null) {
    return findPrimaryColor(name)?.name ?? DEFAULT_THEME_PRIMARY;
}

export function findSurface(name?: string | null) {
    return surfaces.find((surface) => surface.name === name);
}

export function resolveThemeSurfaceName(name?: string | null) {
    return findSurface(name)?.name ?? null;
}

export function getPrimaryPresetExtension(primaryName?: string | null): Preset {
    const color = findPrimaryColor(primaryName) ?? findPrimaryColor(DEFAULT_THEME_PRIMARY);
