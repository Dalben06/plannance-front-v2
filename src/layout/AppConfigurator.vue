...
import { getPrimaryPresetExtension, primaryColors, resolveThemePreset, surfaces, themePresetNames, type ThemePaletteOption } from '@/layout/utils/theme';
...
const presetOptions = ref(themePresetNames);
...
function updateColors(type: string, color: ThemePaletteOption) {
    if (type === 'primary') {
...
function applyTheme(type: string, color: ThemePaletteOption) {
    if (type === 'primary') {
        updatePreset(getPrimaryPresetExtension(layoutConfig.primary));
    } else if (type === 'surface') {
...
function onPresetChange() {
    ...
    const presetValue = resolveThemePreset(preset.value);
    const surfacePalette = surfaces.find((surface) => surface.name === layoutConfig.surface)?.palette;

    $t().preset(presetValue).preset(getPrimaryPresetExtension(layoutConfig.primary)).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
}
</script>
